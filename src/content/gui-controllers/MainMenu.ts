import { GuiController, UiState, CURSOR_CLICK, CURSOR_DOWN, Gui, GuiStates } from "../../ui-state";
...
export const MainMenuGuiController: GuiController<'MainMenu'> = {
    id: "MainMenu",
...
function renderMainMenuUi(ctx: CanvasRenderingContext2D, gameState: GameState, ui: UiState) {
    const { cursorActive, cursorX, cursorY } = ui
    const width = ui.playArea.width
    const height = ui.playArea.height

    ctx.save()

    // Title
    let hPos = (3 * height / 12) + ui.playArea.top
    const halfW = width / 2
    {
        ctx.save()
        ctx.font = '85px serif'
